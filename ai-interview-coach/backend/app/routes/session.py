...
async def start_session(
    ...
    try:
        ...
        # IMPORTANT: this must be an HTTPException, not a bare raise - an
        # unhandled exception here gets caught by Starlette's outer error
        # handler and returned WITHOUT CORS headers (since that layer sits
        # outside CORSMiddleware), which makes the browser report a
        # confusing "CORS blocked" error instead of the real cause.
        raise HTTPException(500, f"Failed to process resume/JD: {str(e)}")
